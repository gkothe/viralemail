	...
	private void processaLogin(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (!(password.equals("-1")) && !(user.equals("-1"))) {
				...
				PreparedStatement stmt = conn.prepareStatement("select id_distribuidora,desc_login,cod_cidade from distribuidora WHERE  Binary desc_login = ? and Binary desc_senha = ? and flag_ativo_master = 'S' ");
